...
class Calls {
  ...
  async approveTransfer(api, args) {
    ...
    return api.tx.assets.approveTransfer(Number(args[0]), args[1], adjustedAmount);
  }
  ...
  async forceTransfer(api, args) {
    ...
    console.log({args, adjustedAmount})
    return api.tx.assets.forceTransfer(Number(args[0]), args[1], args[2], adjustedAmount);
  }
  ...
  async setMetadata(api, args) {
    console.log({args})
    return api.tx.assets.setMetadata(Number(args[0]), args[1], args[2], args[3]);
  }
  ...
  async transferApproved(api, args) {
    const adjustedAmount = await adjustAmount(api, args[0], args[3]);
    console.log({args, adjustedAmount})
    return api.tx.assets.transferApproved(Number(args[0]), args[1], args[2], adjustedAmount);
  }

  async transferKeepAlive(api, args) {
    const adjustedAmount = await adjustAmount(api, args[0], args[2]);
    console.log({args, adjustedAmount})
    return api.tx.assets.transferKeepAlive(Number(args[0]), args[1], adjustedAmount);
  }
